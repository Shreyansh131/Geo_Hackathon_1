...
# Update Embedder to use the FAST model
if 'text_embedder' not in st.session_state:
    # Use 'nomic-embed-text' if you pulled it, otherwise fallback to 'llama3' (slow)
    try:
        st.session_state.text_embedder = OllamaEmbeddings(
            base_url="http://localhost:11434",
            model="nomic-embed-text" # <--- MUUUCH FASTER
        )
    except:
        st.session_state.text_embedder = OllamaEmbeddings(
            base_url="http://localhost:11434",
...
#
#     return docs
def extract_pdf(file_path, enable_ocr=False):
    """
    Optimized PDF extraction.
    OCR is DISABLED by default to increase speed by 20x.
    """
    source = str(file_path)
    docs = []
    ...
    # 2. Tables (Medium Speed)
    # If Camelot is too slow, comment this out and rely on pdfplumber only
    docs.extend(extract_pdf_tables_camelot(source))

    # 3. PDFPlumber (Good balance)
    pdfplumber_docs = extract_pdf_tables_pdfplumber(source)
    docs.extend(pdfplumber_docs)

    # 4. OCR (The Slow Part) - Only run if explicitly enabled
    if enable_ocr:
        st.info("⏳ Running OCR on images (this will take time)...")
        docs.extend(extract_pdf_ocr(source))
...
import os
import time
import gc  # <--- Required for forcing file release


def reset_knowledge_base():
    """
    Fast Reset: Instead of deleting (slow/error-prone),
    we switch to a new unique directory ID.
    """
    # 1. Clear Memory
    ...
    # 3. Kill VectorStore Reference
    if 'vectorstore' in st.session_state:
        del st.session_state['vectorstore']

    # 4. Generate NEW Session ID (Instant "Wipe")
    st.session_state.session_id = str(uuid.uuid4())

    # 5. Re-initialize with NEW Path
    db_path = f"vectorDB/{st.session_state.session_id}"
    os.makedirs(db_path, exist_ok=True)

    if 'text_embedder' in st.session_state:
        st.session_state.vectorstore = Chroma(
...
def load_documents(file_path, enable_ocr=False):
    """
    OPTIMIZED: Defaults to fast PyMuPDF loading.
    Skips slow OCR/Camelot unless specifically requested.
    """
    ext = Path(file_path).suffix.lower()

    if ext == ".pdf":
        # FAST PATH: Use PyMuPDFLoader (Text only, very fast)
        # If you need tables, pdfplumber is called inside extract_pdf_tables_pdfplumber
        docs = []
        ...
        try:
            ...
            # Fallback
            docs.extend(extract_pdf_text_base(file_path))

        # 2. PDFPlumber (Medium speed, good for layout)
        try:
            docs.extend(extract_pdf_tables_pdfplumber(file_path))
        except:
            pass

        # 3. OCR (Only if enabled via UI checkbox)
        if enable_ocr:
            docs.extend(extract_pdf_ocr(file_path))
        ...
        if CSVLoader is None: return []
        loader = CSVLoader(file_path)
        return loader.load()
    elif ext == ".docx":
        if DocxLoader is None: return []
        loader = DocxLoader(file_path)
        return loader.load()
    elif ext in [".png", ".jpg", ".jpeg"]:
        return extract_image(file_path)
...
def chunk_documents(docs):
    """
    OPTIMIZED: Uses larger chunk sizes (4000) to drastically reduce
    embedding time (indexing speed).
    """
    text_splitter_text = RecursiveCharacterTextSplitter(
        chunk_size=4000,  # Increased from ~1000 to 4000
        chunk_overlap=400
    )
    # Keep tables smaller for precision
    ...
    for d in docs:
        ...
        c_type = d.metadata.get("content_type", "text")
        splitter = text_splitter_table if c_type in {"table", "stats"} else text_splitter_text
        split = splitter.split_documents([d])
        chunks.extend(split)
    return chunks
...
# ----------------------------
if selected == "Home":
    st.title("LocoChat: Document Insights")

    # Sidebar: Manual Reset
    ...
    # Toggle OCR
    use_ocr = st.checkbox("Enable OCR (Only for scanned images)", value=False)

    uploaded_file = st.file_uploader("Upload Document", type=["pdf", "docx", "csv", "png", "jpg"], key="main_upload")
    ...
    if uploaded_file:
        ...
        if st.session_state.get('last_loaded_file') != uploaded_file.name:
            ...
            # PROGRESS BAR to track speed
            progress_text = "Operation in progress. Please wait..."
            my_bar = st.progress(0, text=progress_text)

            try:
                # 1. Reset (Instant now)
                my_bar.progress(10, text="Creating new workspace...")
                reset_knowledge_base()
                ...
                # 3. Load & Chunk
                my_bar.progress(30, text="Reading PDF (Fast Mode)...")
                docs = load_documents(file_path, enable_ocr=use_ocr)

                if not docs:
                    st.error("No text found. If this is a scanned PDF, check 'Enable OCR'.")
                    my_bar.empty()
                else:
                    my_bar.progress(50, text="Splitting text...")
                    chunks = chunk_documents(docs)

                    # 4. Embed (The slow part, now tracked)
                    my_bar.progress(60, text=f"Embedding {len(chunks)} chunks... (This depends on your GPU/CPU)")
        ...
        # Chat Input (Only shows if file is loaded)
...
if selected_new == "Summary Generation":
    ...
    col1, col2 = st.columns(2)
    with col1:
        summary_length = st.selectbox("Select summary length:",
                                      ["Short (Bullet Points)", "Medium (1 Page)", "Long (Detailed Report)"])
    with col2:
        summary_type = st.selectbox("Select summary type:",
                                    ["Technical (Engineering)", "Executive (Business)", "General (EL15)"])

    user_topic = st.text_input("Specific topic (Optional - leave empty for full doc summary):")

    if st.button("Generate Summary"):
        if 'vectorstore' not in st.session_state:
            st.error("Please upload a document in 'Home' first.")
        else:
            with st.spinner("Analyzing document and synthesizing summary..."):
                try:
                    # 1. Retrieve Context (Get more chunks for a summary: k=10)
                    retriever = st.session_state.vectorstore.as_retriever(search_kwargs={"k": 10})

                    # 2. Construct a Specific Prompt based on your dropdowns
                    # This forces the LLM to change its output style
                    prompt_text = (
                        f"You are an expert analyst. Generate a {summary_length} summary "
                        f"focusing on {summary_type} details. "
                        f"{f'Focus specifically on: {user_topic}.' if user_topic else 'Cover the entire document.'}\n\n"
                        "Use the provided context to write a coherent narrative. Do not just list facts."
                    )

                    # 3. Run the Chain with the custom prompt
                    # We use 'run' on the chain or re-instantiate a simple chain for this specific task
                    qa_summary = RetrievalQA.from_chain_type(
                        llm=st.session_state.text_llm,
                        chain_type="stuff",
                        retriever=retriever
                    )

                    summary_res = qa_summary.run(prompt_text)
                    ...
                    # 5. Save to PDF
                    pdf_file = f"pdfFiles/summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
                    save_response_to_pdf(summary_res, [], filename=pdf_file, user_query="Summary")

                    with open(pdf_file, "rb") as f:
                        st.download_button("Download Summary PDF", f, file_name=Path(pdf_file).name)
...
# ----------------------------
# ----------------------------------------------------
# Option 3: Nodal Analysis Results (OPTIMIZED & ROBUST)
# ----------------------------------------------------
# ----------------------------------------------------
# ----------------------------------------------------
# ----------------------------------------------------
# Option 3: Nodal Analysis Results (FIXED IMPORTS & LOGIC)
# ----------------------------------------------------
# ----------------------------------------------------
# Option 3: Nodal Analysis Results (NO-CHAIN VERSION)
# ----------------------------------------------------
    ...
    # --- NO BROKEN IMPORTS HERE ---
    # We purposefully removed 'from langchain.chains ...' to stop the crash.

    # We only need these basic ones which usually work fine
    from langchain_community.vectorstores import Chroma
    from langchain_community.embeddings import OllamaEmbeddings
    from langchain_community.llms import Ollama

    st.subheader("⛽ Nodal Analysis Results")
    st.markdown("---")

    # --- 0. AUTO-RECONNECT (Database Fix) ---
    if 'vectorstore' not in st.session_state or st.session_state.vectorstore is None:
        if os.path.exists('vectorDB'):
            try:
                if 'text_embedder' not in st.session_state:
                    st.session_state.text_embedder = OllamaEmbeddings(base_url="http://localhost:11434", model="llama3")
    ...
    if 'text_llm' not in st.session_state:
        st.session_state.text_llm = Ollama(base_url="http://localhost:11434", model="llama3")
    ...
    rag_ready = 'vectorstore' in st.session_state and st.session_state.vectorstore is not None

    # --- 2. DATA RETRIEVAL ---
    if 'nodal_params' in st.session_state and 'nodal_results' in st.session_state:
        st.success("✅ Loaded manual calculation results.")
        ...
        st.info("ℹ️ Auto-calculating from documents...")
        try:
            with st.spinner("Extracting parameters..."):
                # MANUAL RETRIEVAL (Bypassing Retriever Chains)
                # We use similarity_search directly on the vectorstore
                query_text = "reservoir pressure, wellhead pressure, PI, ESP depth, pump curve"
                source_docs = st.session_state.vectorstore.similarity_search(query_text, k=6)
        ...
        st.warning("⚠️ Database not found. Please upload a PDF in 'Home'.")
        st.stop()
    ...
    # --- 4. GENERATE SUMMARY (MANUAL LLM CALL) ---
    summary_text = "Waiting for AI..."

    if sol_flow is not None and rag_ready:
        try:
            with st.spinner("🤖 AI Analyst is writing summary..."):
                # 1. Retrieve Context Manually
                if not source_docs:
                    source_docs = st.session_state.vectorstore.similarity_search("Well summary and operational issues",
                                                                                 k=4)

                # Smash context into a string
                context_str = "\n\n".join([doc.page_content for doc in source_docs])

                # 2. Manual Prompt Construction
                final_prompt = f"""
                You are a Petroleum Engineer. Use the context below to summarize the well status.

                CALCULATED DATA:
                - Flow Rate: {sol_flow_str} m3/hr
                - BHP: {sol_pbh_str} bar

                CONTEXT FROM DOCUMENTS:
                {context_str}

                INSTRUCTION: Write a 3-4 sentence engineering summary based on the above.
                """

                # 3. Direct LLM Call (No Chains!)
                # .invoke() is standard, but some older versions use .predict() or __call__
                try:
                    summary_text = st.session_state.text_llm.invoke(final_prompt)
                except:
                    summary_text = st.session_state.text_llm(final_prompt)

        except Exception as e:
            summary_text = f"AI Error: {str(e)}"
    elif sol_flow is None:
        summary_text = "No operating point found."

    # --- 5. DISPLAY DASHBOARD ---
    st.markdown("### 📊 Executive Summary")
    st.info(summary_text)

    col_op, col_data = st.columns([1, 1])
    with col_op:
        st.markdown("#### 🎯 Operating Point")
        if sol_flow:
            st.metric("Flowrate (Q)", f"{sol_flow_str} m³/hr")
            st.metric("Bottomhole Pressure", f"{sol_pbh_str} bar")
            st.metric("Pump Head", f"{sol_head_str} m")
        else:
            st.error("No intersection found.")

    with col_data:
        st.markdown("#### ⚙️ Key Input Data")
        st.write(f"**Reservoir Pres:** {params.get('reservoir_pressure', '-')} bar")
        st.write(f"**PI:** {params.get('PI', '-')} m³/hr/bar")

    # --- 6. PLOTTING ---
    st.markdown("---")
    st.subheader("📈 IPR vs VLP Curve")
    plot_path = "temp_nodal_plot.png"

    if results.get('flows') is not None and len(results['flows']) > 0:
        fig, ax = plt.subplots(figsize=(10, 6))
        ax.plot(results["flows"], results["p_vlp"], label="VLP", color='blue', linewidth=2)
        ax.plot(results["flows"], results["p_ipr"], label="IPR", color='green', linewidth=2)
        if sol_flow:
            ax.scatter(sol_flow, sol_pbh, color="red", s=150, zorder=5)
            ax.annotate(f"  Q={sol_flow_str}", (sol_flow, sol_pbh), fontweight='bold')
    ...
    if st.button("📄 Export Results to PDF"):
        try:
            op_table = {"Flowrate": f"{sol_flow_str} m3/hr", "BHP": f"{sol_pbh_str} bar", "Head": f"{sol_head_str} m"}
            ...
            param_table = {
                "Reservoir Pres": s('reservoir_pressure'), "PI": s('PI'), "ESP Depth": s('esp_depth'),
                "Fluid Density": s('rho'), "Viscosity": s('mu')
            }
            pdf_name = f"Nodal_Report_{datetime.now().strftime('%H%M%S')}.pdf"
            ...
            save_response_to_pdf(
                ...
                parameter_dict=param_table,
                nodal_plot_path=plot_path if plot_path and sol_flow else None
            )
